...
def Validate_File_Paths():
    ...
    for table_name, file_path in TABLE_DATA.items():
        if os.path.exists(file_path):
            print(f"✓ Found {table_name} file: {os.path.basename(file_path)}")
        else:
            print(f"✗ Missing {table_name} file: {file_path}")
...
def Add_Keys_And_Relationships():
    ...
    try:
        ...
        try:
            ...
            print(f"Warning: Could not add foreign key from players to teams: {e}")
        ...
        try:
            ...
            print(f"Warning: Could not add foreign key from box_score to players: {e}")
            print("  This could be due to missing references or data inconsistencies")
        ...
        try:
            ...
            print(f"Warning: Could not add foreign key from box_score to teams: {e}")
            print("  This could be due to missing references or data inconsistencies")
...
def Create_Database():
    ...
    for table_name, csv_file in TABLE_DATA.items():
        try:
            ...
            uppercase_columns = [col for col in original_columns if col.isupper() or col.upper() == col]
            
            if table_name == 'players' and 'PERSON_ID' not in uppercase_columns:
                person_id_candidates = [col for col in uppercase_columns if 'ID' in col]
                if person_id_candidates:
                    df = df.rename(columns={person_id_candidates[0]: 'PERSON_ID'})
                    uppercase_columns = list(df.columns)
                    print(f"Renamed {person_id_candidates[0]} to PERSON_ID for consistency")
            
            dropped_columns = [col for col in original_columns if col not in uppercase_columns]
            if dropped_columns:
                df = df[uppercase_columns]
                print(f"Dropped {len(dropped_columns)} non-uppercase columns to avoid duplicates")
                
            if 'TO' in df.columns:
                df = df.rename(columns={'TO': 'turnovers'})
                print("Renamed 'TO' column to 'turnovers' (MySQL reserved keyword)")
            ...
            if table_name == 'box_score' and 'GAME_ID' in df.columns and 'PLAYER_ID' in df.columns:
                ...
                if duplicate_count > 0:
                    print(f"Removed {duplicate_count} duplicate rows from box_score (duplicate GAME_ID, PLAYER_ID combinations)")
    ...
    try:
        if 'players' in dfs and 'teams' in dfs:
            ...
            if 'TEAM_ID' in players_df and 'TEAM_ID' in teams_df and 'TEAM_NAME' in teams_df and 'TEAM_NAME' in players_df:
                print("\nFinding and fixing inconsistencies between players and teams...")
                
                valid_team_ids = set(teams_df['TEAM_ID'].unique())
                invalid_team_ids_mask = ~players_df['TEAM_ID'].isin(valid_team_ids)
                invalid_count = invalid_team_ids_mask.sum()
                
                if invalid_count > 0:
                    print(f"Found {invalid_count} players with invalid TEAM_ID references")
                    ...
                    for idx in players_df[invalid_team_ids_mask].index:
                        ...
                        if player_team_name in team_name_to_id:
                            players_df.loc[idx, 'TEAM_ID'] = team_name_to_id[player_team_name]
                            fixed_count += 1
                    
                    print(f"Fixed {fixed_count} players by matching team names")
                    
                    remaining_invalid = ~players_df['TEAM_ID'].isin(valid_team_ids)
                    if remaining_invalid.sum() > 0:
                        players_df = players_df[~remaining_invalid]
                        print(f"Removed {remaining_invalid.sum()} players with unresolvable team references")
        ...
        if 'box_score' in dfs and 'players' in dfs:
            ...
            if 'PLAYER_ID' in box_score_df and 'PERSON_ID' in players_df and 'PLAYER_NAME' in box_score_df and 'PLAYER_NAME' in players_df:
                print("\nFinding and fixing inconsistencies between box_score and players...")
                
                valid_player_ids = set(players_df['PERSON_ID'].unique())
                invalid_player_ids_mask = ~box_score_df['PLAYER_ID'].isin(valid_player_ids)
                invalid_count = invalid_player_ids_mask.sum()
                
                if invalid_count > 0:
                    print(f"Found {invalid_count} box score entries with invalid PLAYER_ID references")
                    
                    player_name_to_id = dict(zip(players_df['PLAYER_NAME'], players_df['PERSON_ID']))
                    
                    fixed_count = 0
                    for idx in box_score_df[invalid_player_ids_mask].index:
                        box_score_player_name = box_score_df.loc[idx, 'PLAYER_NAME']
                        if box_score_player_name in player_name_to_id:
                            box_score_df.loc[idx, 'PLAYER_ID'] = player_name_to_id[box_score_player_name]
                            fixed_count += 1
                    
                    print(f"Fixed {fixed_count} box score entries by matching player names")
                    
                    remaining_invalid = ~box_score_df['PLAYER_ID'].isin(valid_player_ids)
                    if remaining_invalid.sum() > 0:
                        box_score_df = box_score_df[~remaining_invalid]
                        print(f"Removed {remaining_invalid.sum()} box score entries with unresolvable player references")
        ...
        if 'box_score' in dfs and 'teams' in dfs:
            ...
            if 'TEAM_ID' in box_score_df and 'TEAM_ID' in teams_df and 'TEAM_ABBREVIATION' in box_score_df and 'ABBREVIATION' in teams_df:
                print("\nFinding and fixing inconsistencies between box_score and teams...")
                
                valid_team_ids = set(teams_df['TEAM_ID'].unique())
                invalid_team_ids_mask = ~box_score_df['TEAM_ID'].isin(valid_team_ids)
                invalid_count = invalid_team_ids_mask.sum()
                
                if invalid_count > 0:
                    print(f"Found {invalid_count} box score entries with invalid TEAM_ID references")
                    
                    team_abbrev_to_id = dict(zip(teams_df['ABBREVIATION'], teams_df['TEAM_ID']))
                    team_city_to_id = dict(zip(teams_df['CITY'], teams_df['TEAM_ID']))
                    
                    fixed_count = 0
                    for idx in box_score_df[invalid_team_ids_mask].index:
                        team_abbrev = box_score_df.loc[idx, 'TEAM_ABBREVIATION']
                        team_city = box_score_df.loc[idx, 'TEAM_CITY'] if 'TEAM_CITY' in box_score_df.columns else None
                        
                        if team_abbrev in team_abbrev_to_id:
                            box_score_df.loc[idx, 'TEAM_ID'] = team_abbrev_to_id[team_abbrev]
                            fixed_count += 1
                        elif team_city and team_city in team_city_to_id:
                            box_score_df.loc[idx, 'TEAM_ID'] = team_city_to_id[team_city]
                            fixed_count += 1
                    
                    print(f"Fixed {fixed_count} box score entries by matching team abbreviation or city")
                    
                    remaining_invalid = ~box_score_df['TEAM_ID'].isin(valid_team_ids)
                    if remaining_invalid.sum() > 0:
                        box_score_df = box_score_df[~remaining_invalid]
                        print(f"Removed {remaining_invalid.sum()} box score entries with unresolvable team references")
    ...
    for table_name in table_order:
        if table_name in dfs:
            ...
            try:
                ...
                print(f"Successfully created {table_name} table with {len(df)} rows and {len(df.columns)} columns")
            except Exception as e:
                print(f"Error saving {table_name} to database: {e}")
                return False
    
    if not Add_Keys_And_Relationships():
        print("Failed to add keys and relationships. Database created but without proper constraints.")
        return False
        ...
        for table in tables:
            ...
            for table in tables:
                ...
                for table in tables:
                    ...
                    if fks:
                        for fk in fks:
                            print(f"  - {fk[0]}.{fk[1]} → {fk[3]}.{fk[4]} (via {fk[2]})")
